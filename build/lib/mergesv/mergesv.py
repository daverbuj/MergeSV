...
def is_recip_overlap(ref, sv, ro, edge_size):
...
def merge_and_delete(merged_list, svlist, values):
	...
	# used after the first merge loop
	elif len(values[0]) == 7:
		if len(values) == 1:
...
def merge_list(sv_list, ro, edge_size):
	...
	while len(sv_list) > 0:
		...
		for sv in sv_list:
			...
			elif is_recip_overlap(ref, sv, ro, edge_size):
				to_merge.append(sv)
				if i == ilen:
					merge_and_delete(merged_list, sv_list, to_merge)
					break
...
#				split_dict[chrom] = {'DEL':[], 'DUP':[], 'INV':[]}
	...
	## Write the final merged list to file

	for sv in final_merged:
		if not args.c:
			out_writer.writerow( sv[:-1] )
		else:
			out_writer.writerow( sv )
	
	out_fh.close()

	fin = 'DONE: {}   '.format(outfile)
	print('#'*(len(fin)+6) + '\n   {}\n'.format(fin) + '#'*(len(fin)+6))